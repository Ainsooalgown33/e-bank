...
myquery = "INSERT INTO ubabank (FirstName, LastName, contact, line, account_number, password, account_balance) VALUES (%s, %s, %s, %s, %s, %s, %s)"
def operation():
    global func
    print(""" Welcome to UBA bank, Flow with the money. what would you like to do?!!!
...
def membership():
    ...
    conn.commit()
    print(mycursor.rowcount, "bank record updated successfully, welcome to UBA bank " + firstName  +  lastName, " your account number is " + str(account_number))

def login():
    account_number = input("Enter your account number >")
    password = input("Enter your password >")
    query = "SELECT * FROM ubabank WHERE account_number=%s AND password=%s"
    ...
    if result is not None:
        ...
        if option == "1":
            ...
            query = "UPDATE ubabank SET account_balance=%s WHERE account_number=%s"
            ...
            query = "SELECT * FROM ubabank WHERE account_number=%s UNION SELECT * FROM firstbank WHERE account_number=%s UNION SELECT * FROM accessbank WHERE account_number=%s"
            ...
            if receiver_result is not None:
                ...
                if int(transfer_amount) > int(account_balance):
                    ...
                    query1 = "UPDATE ubabank SET account_balance=%s WHERE account_number=%s"
                    ...
                    query3 = "UPDATE accessbank SET account_balance=%s WHERE account_number=%s"
                    val3 = (str(new_sender_balance), int(account_number))
                    mycursor.execute(query3, val3)

                
                    query4 = "UPDATE ubabank  SET account_balance=%s WHERE account_number=%s"
                    ...
                    query6 = "UPDATE accessbank  SET account_balance=%s WHERE account_number=%s"
